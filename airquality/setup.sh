...
if [ -n "${PROJECT_PATH:-}" ]; then
  ...
  if ! command -v uv >/dev/null 2>&1; then
    if ! command -v curl >/dev/null 2>&1; then
      echo "❌ curl is required to install uv but was not found"
    ...
    # Add uv to PATH for this session (installer may use either location)
  ...
  if [ "$RECREATE_VENV" -eq 1 ]; then
    ...
    if ! uv venv "$VENV_DIR" --python "$PYTHON_BIN"; then
